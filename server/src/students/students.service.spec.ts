...
const testStudents = [ testStudent, {...testStudent, _id: "testid2"}, {...testStudent, _id: "testid3"}]
...
describe('StudentService', () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: getModelToken(Student.name), 
          useValue: {
            create: jest.fn(dto => ({...dto, _id: "generatedid"})),
  ...
  it('should update student', async () => {
    const updated = await service.update("testid", {fullName: "Changed Name"})
    expect(model.findById).toBeCalledWith("testid")
  ...
  it('should throw for findOne when not found', () => {
    expect(async () => { await service.findOne("notexisting") }).rejects.toThrow(NotFoundException)
  })

  it('should throw for update when not found', () => {
    expect(async () => { await service.update("notexisting", {}) }).rejects.toThrow(NotFoundException)
  })

  it('should throw for remove when not found', () => {
    expect(async () => { await service.remove("notexisting") }).rejects.toThrow(NotFoundException)
  })
});
